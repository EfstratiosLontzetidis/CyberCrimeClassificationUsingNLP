...
def case_to_owl_jaccard(cases_prefix):
    ...
    print("****Algorithm: JACCARD SIMILARITY********")
    # load ontology
    ...
    for file in os.listdir(cases_prefix):
        ...
        for obj in characteristics_prefix:
            ...
            for file2 in os.listdir(obj):
                ...
                second = set(documents[1].split())
                intersection=first.intersection(second)
                jacccard_similarity=float(len(intersection)) / (len(first) + len(second) - len(intersection))
                if (jacccard_similarity > 0.04):
                    if counter == 0:
        ...
        print("\n")
    print(
        "Now you can import the new ontology 'Cyber_crime_ontology_with_new_crime_cases.owl' in Protege and start the reasoner Hermi to classify your cases to their cyber crime category (see individuals by type inferred).")
